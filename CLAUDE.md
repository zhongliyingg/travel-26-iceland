...
- Past edits have accidentally removed: the Lava Show (Day 5) and the Perlan museum (Day 4). The 5-stop 花少5 South Coast route is also fragile — stops 1–4 sit on Day 6 and stop 5 (Dyrhólaey) on Day 8. Be especially careful around these sections.
- **Deliberate removals — do not "restore" them.** The spa day (Blue Lagoon / Hvammsvík) was removed on purpose in commit `46ea707` when the Iceland leg was restructured into 3 bases. Hvammsvík now survives only in the archived `Iceland_Family_Trip_2026_Bilingual_Enhanced_20260411.html` and must not be reintroduced into `index.html` or `content/`. The one correct "Blue Lagoon" mention in the live files is the skincare shop on Laugavegur in the Day 11 shopping list — not the lagoon.
